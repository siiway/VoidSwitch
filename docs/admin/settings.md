# 设置

**设置**页面包含运维阈值和间隔，运行时生效 — 无需重新部署。**管理员可以查看**设置（只读）；
**编辑**和**"立即清理日志"**操作是**仅限所有者**的。

设置按类型通用渲染：布尔值显示为开关，数字显示为微调按钮，字符串显示为输入框。它们按分组排列。

## 分组

- **节点与路由** — 代理切换总开关（`proxy_switching_enabled`）：`true` = 节点/节点组路由系统开启；
  `false` = 关闭路由系统，每个请求走 `static_proxy_url`（或环境变量）。
  路由系统相关设置含 `node_default_probe_url`（节点组未单独设置时使用的探测 URL，也是唯一的探测 URL 设置）、`node_probe_interval_seconds`
  （空闲健康检查间隔）、`node_rank_alpha/beta/gamma`（节点动态排序的权重）、
  `node_rank_ewma_half_life_seconds`（节点 EWMA 延迟的衰减半衰期）、`max_retries`（每请求最大重试）
  和 `max_proxy_failures`（节点禁用前的失败阈值）。
- **密钥与余额** — 密钥失败限制、零余额密钥自动禁用以及余额探测/重新扫描节奏和速率。
- **速率限制** — 默认恢复窗口和任何单次冷却的上限。
- **上游路由** — 上游选择与排序方式、评分权重、健康样本门槛、每个上游尝试的密钥数，
  以及触发冷却的状态码、冷却时长/退避和全部候选冷却时的行为。状态码列表使用逗号分隔，
  重试时间响应头每行一个。
- **超时与重试** — 连接 / 请求 / 流空闲超时和重试预算，外加**响应超时**（`response_timeout_seconds`）：
  这是整个请求（含流式）的**硬性墙钟上限**。超过后连接会被**强制切断**，日志行标记为**已切断**。
  流式请求此前只有空闲超时，没有总时长上限——慢速滴流的连接可能一直显示"进行中"；此设置兜底
  切断这类悬挂连接（`0` = 禁用）。服务启动时也会把超过该时长的残留"进行中"记录统一标记为**已切断**。
- **登录与会话** — **登录态时长**（`session_ttl_minutes`，分钟）：仪表盘会话 JWT 的有效期。
  `0`（留空）= **遵循 Prism 登录时返回的 `expires_in`**；设置为大于等于 **60** 分钟时使用该值；
  若 Prism 未返回且未设置，则回退到服务端配置的 `session_ttl_minutes`。保存时小于 60 的非零值会被拒绝。
- **日志与保留** — 页面大小、自动清理开关和间隔，以及请求/审计/调试日志的保留窗口
  （`0` = 永久保留）。所有者还可以**立即清理**。调试日志保留窗口
  （`debug_log_retention_days`）过期后会剥离行上的详细调试字段（请求/响应头和体、每次尝试记录），
  也包括为出错请求（上游 4xx / 5xx）强制记录的响应头和响应体。此分组还包含**热力图数据保留天数**
  （`heatmap_retention_days`）：控制台活跃度热力图所依赖的每日用量汇总的保留窗口，独立于请求日志保留。
  默认 365 天；`0` 表示永久保留，非零值不得少于 365 天（保存时会校验）。
- **OpenCode 默认值** — 向插件发布的默认模型和小型模型。
- **模型与目录** — models.dev 占位元数据的同步间隔（`models_dev_sync_interval_minutes`，`0` = 禁用）。
- **平台信息** — 控制台上显示多少条公告后才显示"查看全部"（`announcements_home_count`），以及
  聊天页空状态里的**聊天预设问题**（`chat_preset_questions`，每行一个）。
- **速率限制** — 控制台管理操作固定限流：每用户 20 秒内最多 30 个请求（写死，不可配置）。
  OpenAI / Anthropic 网关调用（`/v1/chat/completions`、`/v1/messages`）的限流
  **按身份组配置**，见[身份组](/admin/role-groups)：默认 30 秒内最多 30 个请求，
  内置 moderator 组默认 30 秒内最多 50 个请求。限流仍按用户计数；加入多个身份组的成员，
  只要任一组仍有额度即可继续调用。

## 注意事项

- 仅在特定条件下才有效的设置在其他情况下会被隐藏（例如静态代理 URL 仅在**代理切换/路由关闭**时显示，
  节点池调优设置仅在路由系统开启时显示）。
- **models.dev 同步间隔**（`models_dev_sync_interval_minutes`）控制暴露模型的 models.dev 占位元数据
  多久刷新一次（默认 `1440` = 每天）。
- 更改在保存时生效；后台任务在其下一次 tick 时获取新的间隔。

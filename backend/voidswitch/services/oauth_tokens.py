...
def extract_code(raw: str) -> tuple[str, str | None]:
    ...
    # Full or partial URL carrying query parameters (a bare ``code=…&state=…``
    # string has no scheme and no leading ``?``, so grab the tail after the last
    # ``?`` just like the xAI parser does).
